...
class EfficientNet(nn.Module):
    ...
    def __init__(self,blocks_args=None, global_params=None):
        ...
        self.decode0 = nn.Sequential(
            ...
            nn.Conv2d(16, self._global_params.num_classes, kernel_size=1,bias=False),
            )
    ...
    def forward(self, inputs):
        encoder = []
        oriencoder = self.extract_features(inputs)
        encoder.append(oriencoder[1]) 
        encoder.append(oriencoder[3])  
        encoder.append(oriencoder[5])  
        encoder.append(oriencoder[11])  
        encoder.append(oriencoder[16])
       
        d4 = self.decode4(encoder[4], encoder[3])  # 512,16,16
        #print(d4.shape)
        d3 = self.decode3(d4, encoder[2]) # 256,32,32
        #print(d3.shape)
        d2 = self.decode2(d3, encoder[1]) # 128,64,64
        #print(d2.shape)
        d1 = self.decode1(d2, encoder[0]) # 64,128,128
        #print(d1.shape)
        out = self.decode0(d1)     # 64,256,256
        return out
...
#  Model  = EfficientNet.from_name('efficientnet-b0',num_classes = 6)